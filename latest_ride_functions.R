...
get_latest_ride_rankings <- function(con, activity_id) {
  if (length(activity_id) == 0L || is.na(activity_id[[1]])) return(tibble::tibble())
  ...
  activity_ranks <- DBI::dbGetQuery(
    ...
    params = list(
      as.character(activity_id[[1]]), METRES_PER_MILE,
      as.character(activity_id[[1]]), METRES_PER_MILE
    )
  ) |>
    tibble::as_tibble()
...
draw_latest_ride_power_curve <- function(efforts) {
  efforts <- efforts |>
    dplyr::filter(!is.na(.data$duration_seconds), !is.na(.data$peak_value), .data$duration_seconds > 0)
  if (nrow(efforts) == 0L) {
    return(latest_ride_notice("A ride-only Gold power curve is unavailable for this ride."))
  }
  ...
  plotly::plot_ly(
    efforts,
    x = ~duration_seconds, y = ~peak_value,
    type = "scatter", mode = "lines+markers",
    line = list(color = "#17324D", width = 3),
    marker = list(color = "#F2A46F", size = 6, line = list(color = "#17324D", width = 1)),
    text = ~paste0(format_latest_effort_duration(duration_seconds), ": ", round(peak_value), " W"),
    hovertemplate = "%{text}<extra></extra>"
  ) |>
    plotly::layout(
      xaxis = list(type = "log", title = "Duration", tickvals = tick_values, ticktext = tick_labels),
      yaxis = list(title = "Best power (W)", rangemode = "tozero"),
      margin = list(l = 55, r = 15, b = 45, t = 10), showlegend = FALSE
    )
}
...
build_latest_ride_hr_zones <- function(streams, max_hr_bpm = get_latest_ride_hr_max()) {
  if (nrow(streams) == 0L || !"heartrate_bpm" %in% names(streams) ||
      all(is.na(streams$heartrate_bpm)) || is.na(max_hr_bpm)) return(tibble::tibble())

  breaks <- c(0, .6, .7, .8, .9, Inf) * max_hr_bpm
  streams |>
    dplyr::filter(!is.na(.data$heartrate_bpm)) |>
    dplyr::mutate(
      zone = cut(.data$heartrate_bpm, breaks = breaks, labels = paste0("Z", 1:5), right = FALSE),
...
draw_latest_ride_hr_zones <- function(streams, max_hr_bpm = get_latest_ride_hr_max()) {
  if (!isTRUE(latest_ride_stream_availability(streams)[["heart_rate"]])) {
    return(latest_ride_notice("Heart-rate telemetry is unavailable for this ride."))
  }
  if (is.na(max_hr_bpm)) {
    return(latest_ride_notice("Set CYCLING_ANALYTICS_HR_MAX_BPM to display temporary presentation-only HR zones."))
  }
  zones <- build_latest_ride_hr_zones(streams, max_hr_bpm)
  colours <- c("#DDE8EF", "#BCD4E3", "#88B2CC", "#F2C38F", "#D97852")
  plotly::plot_ly(
    zones, x = ~minutes, y = ~zone, type = "bar", orientation = "h",
    marker = list(color = colours, line = list(color = "#17324D", width = 1)),
    text = ~paste0(round(minutes), " min"), textposition = "auto",
    hovertemplate = "%{y}: %{x:.1f} min<extra></extra>"
  ) |>
    plotly::layout(
      xaxis = list(title = "Time (minutes)"), yaxis = list(title = "", autorange = "reversed"),
      margin = list(l = 40, r = 15, b = 42, t = 10), showlegend = FALSE
    )
}

ordinal <- function(value) {
  vapply(as.integer(value), function(one_value) {
...
latest_ride_highlights <- function(rankings) {
  if (nrow(rankings) == 0L) return(latest_ride_notice("No comparative highlights are available for this ride."))
  ...
  candidates <- rankings |>
    ...
    dplyr::mutate(
      ...
      label = dplyr::case_when(
        .data$ranking_type == "distance" ~ paste0(ordinal(.data$achievement_rank), " longest ride this year"),
        .data$ranking_type == "speed_50_miles" ~ paste0(ordinal(.data$achievement_rank), " fastest 50+ mile ride this year"),
        .data$ranking_type == "power" ~ paste0(
          ordinal(.data$achievement_rank), " best ",
          vapply(.data$duration_seconds, format_latest_effort_duration, character(1)),
          " power this year · ", round(.data$metric_value), " W"
        ),
        TRUE ~ NA_character_
      )
    ) |>
    dplyr::filter(!is.na(.data$label), .data$achievement_rank <= 10) |>
    dplyr::arrange(.data$priority, .data$achievement_rank, dplyr::desc(.data$duration_seconds)) |>
  ...
  htmltools::div(
    ...
    lapply(candidates$label, function(label) htmltools::div(class = "latest-ride-highlight", label))
  )
}
...
format_latest_effort_duration <- function(seconds) {
  vapply(as.numeric(seconds), function(one_duration) {
    if (one_duration %% 3600 == 0) return(sprintf("%g hr", one_duration / 3600))
    if (one_duration %% 60 == 0) return(sprintf("%g min", one_duration / 60))
